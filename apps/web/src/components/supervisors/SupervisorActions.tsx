...
import { deactivateSupervisor } from '@/lib/actions/supervisors';
...
export function SupervisorActions({ supervisorId }: { supervisorId: string }) {
  const [isPending, startTransition] = useTransition();

  const handleDeactivate = () => {
    startTransition(async () => {
      await deactivateSupervisor(supervisorId);
    });
  };
