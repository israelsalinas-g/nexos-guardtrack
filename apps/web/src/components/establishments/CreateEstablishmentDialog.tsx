...
export function CreateEstablishmentDialog() {
  ...
  const [state, action, isPending] = useActionState<State, FormData>(createEstablishment, null);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="rounded-2xl border-border/50 bg-card sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-lg font-bold tracking-tight">Nuevo Establecimiento</DialogTitle>
        </DialogHeader>
        ...
        <form action={action} className="space-y-4 pt-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="direccion"
              name="direccion"
              placeholder="Ej: Col. Kennedy, Blvd. Morazán"
              className="rounded-xl bg-secondary/50 border-border/50"
            />
          </div>
